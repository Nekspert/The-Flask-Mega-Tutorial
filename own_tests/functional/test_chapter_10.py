...
def test_reset_password_with_invalid_token(client):
    response = client.get("/auth/reset_password/1")
    assert response.status_code == 302

    response = client.get("/auth/reset_password/1", follow_redirects=True)
    assert response.status_code == 200
    assert len(response.history) == 2
    paths = [resp.request.path for resp in response.history]
    assert "/auth/reset_password/1" == paths[0] and "/index" == paths[1]
